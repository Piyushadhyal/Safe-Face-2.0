...
from face_processing import auto_register
...
def is_real_face(face_img, history_buffer, model, IMG_SIZE=128):
    try:
        ...
        avg_pred = sum(history_buffer) / len(history_buffer)
        return avg_pred > 0.6
...
class RecognitionEngine:
    ...
    def log_intruder(self, frame: np.ndarray) -> str:
        now   = datetime.now()
...
# ══════════════════════════════════════════════════════
#  GUI APPLICATION  (unchanged from your original)
# ══════════════════════════════════════════════════════
...
class App(tk.Tk):
    ...
    def _add_student_window(self, parent):
        ...
        def select_folder():
            ...
            passout_year = simpledialog.askinteger("Passout Year", "Enter Passout Year (e.g., 2026):")
    ...
    def _manage_students(self):
        ...
        tk.Label(win, text="Enter Roll Number", bg="#1e1e2e", fg="white").pack(pady=5)
        entry = tk.Entry(win)
        entry.pack(pady=5)

        def add():
            messagebox.showinfo("Info", "Use 'Add Student' feature from your system (camera/upload)")

        def delete():
            roll = entry.get().strip()
            if not roll:
                messagebox.showwarning("Error", "Enter roll number")
                return
            if messagebox.askyesno("Confirm Delete", f"Delete student {roll}?"):
                ok = self.engine.delete_student(roll)
                if ok:
                    self._log(f"✅ Student {roll} added successfully", "known")
                    self._reload_faces()
                else:
                    self._log("❌ Invalid roll number entered", "intruder")

        tk.Button(win, text="Add Student", command=add, bg="#a6e3a1").pack(pady=5)
        tk.Button(win, text="Delete Student", command=delete, bg="#f38ba8").pack(pady=5)
    ...
    def _video_loop(self):
        ...
        while True:
            ...
            ret, frame = cap.read()
            if not ret or not self.running or local_session != self._session_id:
                break
            ...
            # ── detect every other frame (same logic as before) ──
            if process_this_frame:
                last_locs, last_encs = get_face_locations_and_encodings(rgb)

            process_this_frame = not process_this_frame

            for (top, right, bottom, left), enc in list(zip(last_locs, last_encs))[:2]:
                ...
                # ── liveness ──
                if not is_real_face(face_roi, self._liveness_history, liveness_model):
                    is_real = True
                    if frame_count % 5 == 0:
                        is_real = is_real_face(face_roi, self._liveness_history, liveness_model)
                    if not is_real:
                        ...
                        if now - self._last_intruder_time > self.INTRUDER_COOLDOWN:
                            ...
                            self.engine.log_intruder(frame)
                ...
                if hasattr(self, "_last_enc") and frame_count % 2 != 0:
                    enc = self._last_enc
                else:
                    self._last_enc = enc

                if self.engine.avg_encodings:
                    # enc already normalised by get_face_locations_and_encodings()
                ...
                corner_len, thickness = 20, 3
                cv2.line(display, (left,  top),    (left + corner_len, top),    box_color, thickness)
                cv2.line(display, (left,  top),    (left,  top + corner_len),   box_color, thickness)
                cv2.line(display, (right, top),    (right - corner_len, top),   box_color, thickness)
                cv2.line(display, (right, top),    (right, top + corner_len),   box_color, thickness)
                cv2.line(display, (left,  bottom), (left + corner_len, bottom), box_color, thickness)
                cv2.line(display, (left,  bottom), (left,  bottom - corner_len),box_color, thickness)
                cv2.line(display, (right, bottom), (right - corner_len, bottom),box_color, thickness)
                cv2.line(display, (right, bottom), (right, bottom - corner_len),box_color, thickness)
                ...
                cv2.rectangle(display, (left, top - th - 12), (left + tw + 10, top), box_color, -1)
                cv2.putText(display, label, (left + 5, top - 5), font, fs, (0, 0, 0), ft)
                ...
                if is_known:
                    ...
                    if self._face_history[name] >= 5:
                        ...
                        if current_time - last_time > self.COOLDOWN_SECONDS:
                            self._last_seen_time[name] = current_time
                            self.engine.log_arrival(name)
                            self.after(0, lambda n=name: self._log(f"✓ {n} logged.", "known"))
                else:
                    self._face_history.clear()

            # ── timestamp watermark ──
            ts_text = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cv2.putText(display, ts_text, (10, display.shape[0] - 10),
